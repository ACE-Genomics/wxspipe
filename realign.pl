...
foreach my $pollo (@mlist){
	...
	$cdata{command}.= "$gatk  SortSam -I $tmpdir/$pollo"."_alignment.sam -O $tmpdir/$pollo"."_salignment.sam  -SORT_ORDER queryname\n";
	...
	$cdata{command}.= "$gatk AddOrReplaceReadGroups -I  $tmpdir/$pollo"."_merged.bam -O $output_dir/$pollo.sam -ID $params{ID} -PL $params{PL} -LB $params{LB} -PU $params{ID} -SM $params{SM}\n";
