...
foreach my $pollo (sort keys %pollos){
	...
	if (-f $pollos{$pollo} and $go){
		...
		$ptask{command} .= "$epaths{bwa} -R ".$rg." $ref_fa $pollos{$pollo} $another | $epaths{gatk} SortSam -I /dev/stdin -O $tdir/$pollo"."_sorted.bam --SORT_ORDER coordinate --CREATE_INDEX true  --TMP_DIR $tmp_shit\n";
		...
		$ptask{command} = "$epaths{gatk} BaseRecalibrator -I $tdir/$pollo"."_rmdups.bam -R $ref_fa".(($mode eq 'wgs')?' ':" -L $unions ")."--known-sites $dpaths{ref_dir}/$dpaths{known1} --known-sites $dpaths{ref_dir}/$dpaths{known2} --known-sites $dpaths{ref_dir}/$dpaths{dbsnp} -O $tdir/$pollo"."_recal_data.table\n";
	      	$ptask{dependency} = "afterok:$jid";
		$jid = send2slurm(\%ptask);
		...
		$ptask{command} = "$epaths{gatk} ApplyBQSR -R $ref_fa -I $tdir/$pollo"."_rmdups.bam".(($mode eq 'wgs')?' ':" -L $unions ")."-bqsr-recal-file $tdir/$pollo"."_recal_data.table -O $tdir/$pollo"."_recal.bam\n";
		$ptask{dependency} = "afterok:$jid";
		...
		$ptask{command} = "$epaths{gatk} AnalyzeCovariates -bqsr $tdir/$pollo"."_recal_data.table --plots $rdir/$pollo"."_AnalyzeCovariates.pdf\n";
		$ptask{dependency} = "afterok:$jid";
		...
		unless ($mode eq 'wgs'){
			...
			$ptask{command} = "$epaths{gatk} DepthOfCoverage -I $tdir/$pollo"."_recal.bam -O $rdir/$pollo"."_raw_wes_metrics.txt -R $ref_fa".(($mode eq 'wgs')?' ':" -L $unions ")."--summary-coverage-threshold 10 --summary-coverage-threshold 15 --summary-coverage-threshold 20 --summary-coverage-threshold 30 --summary-coverage-threshold 40 --summary-coverage-threshold 50 --summary-coverage-threshold 60 --summary-coverage-threshold 70 --summary-coverage-threshold 80 --summary-coverage-threshold 90 --summary-coverage-threshold 100 --omit-depth-output-at-each-base true --omit-interval-statistics true --omit-locus-table true\n";
			...
			$ptask{command} = "$epaths{gatk} DepthOfCoverage -I $tdir/$pollo"."_recal.bam -O $rdir/$pollo"."_wes_metrics.txt -R $ref_fa".(($mode eq 'wgs')?' ':" -L $unions ")."--summary-coverage-threshold 10 --summary-coverage-threshold 15 --summary-coverage-threshold 20 --summary-coverage-threshold 30 --summary-coverage-threshold 40 --summary-coverage-threshold 50 --summary-coverage-threshold 60 --summary-coverage-threshold 70 --summary-coverage-threshold 80 --summary-coverage-threshold 90 --summary-coverage-threshold 100 --omit-depth-output-at-each-base true --omit-interval-statistics true --omit-locus-table true --min-base-quality 20 -RF MappingQualityReadFilter --minimum-mapping-quality 20\n";
			$ptask{dependency} = "afterok:$jid0";
			$jid1 = send2slurm(\%ptask);
			push @ljobids, $jid1;
			...
			$ptask{command} = "$epaths{gatk} DepthOfCoverage -I $tdir/$pollo"."_recal.bam -O $rdir/$pollo"."_padded_wes_metrics.txt -R $ref_fa".(($mode eq 'wgs')?' ':" -L $unions ")."--summary-coverage-threshold 10 --summary-coverage-threshold 15 --summary-coverage-threshold 20 --summary-coverage-threshold 30 --summary-coverage-threshold 40 --summary-coverage-threshold 50 --summary-coverage-threshold 60 --summary-coverage-threshold 70 --summary-coverage-threshold 80 --summary-coverage-threshold 90 --summary-coverage-threshold 100 --omit-depth-output-at-each-base true --omit-interval-statistics true --omit-locus-table true -ip 100 --min-base-quality 20 -RF MappingQualityReadFilter --minimum-mapping-quality 20\n";
			$ptask{dependency} = "afterok:$jid0";
			$jid1 = send2slurm(\%ptask);
			push @ljobids, $jid1;
		}else{
			...
			$ptask{filename} = $slurmdir.'/'.$pollo.'_collectWgsMetrics.sh';
			$ptask{output} = $slurmdir.'/'.$pollo.'_collectWgsMetrics.out';
			$ptask{command} = "$epaths{gatk} CollectWgsMetrics -I $tdir/$pollo"."_recal.bam -O $rdir/$pollo"."_wgs_metrics.txt -R $ref_fa\n";
			$ptask{dependency} = "afterok:$jid0";
			$jid1 = send2slurm(\%ptask);
			push @ljobids, $jid1;
			$ptask{job_name} = $pollo.'_collectRawMetrics';
			$ptask{filename} = $slurmdir.'/'.$pollo.'_collectRawMetrics.sh';
			$ptask{output} = $slurmdir.'/'.$pollo.'_collectRawMetrics.out';
			$ptask{command} = "$epaths{gatk} CollectRawWgsMetrics -I $tdir/$pollo"."_recal.bam -O $rdir/$pollo"."_raw_wgs_metrics.txt -R $ref_fa\n";
			$ptask{dependency} = "afterok:$jid0";
			$jid1 = send2slurm(\%ptask);
			push @ljobids, $jid1;
		}
		...
		$ptask{command} = "$epaths{gatk} HaplotypeCaller -R $ref_fa".(($mode eq 'wgs')?' ':" -L $unions ")."-I $tdir/$pollo"."_recal.bam -G StandardAnnotation -G AS_StandardAnnotation -ERC GVCF --dbsnp $dpaths{ref_dir}/$dpaths{dbsnp} -O $rdir/$pollo"."_raw.snps.indels.g.vcf.gz\n"; 
